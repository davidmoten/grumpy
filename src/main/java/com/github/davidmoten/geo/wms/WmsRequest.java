...
public class WmsRequest {
	...
	public WmsRequest(List<String> layers, List<String> queryLayers,
			List<String> styles, ProjectorBounds bounds, String format,
			int width, int height, boolean transparent, Color backgroundColor,
			String version, String infoFormat, Map<String, String> parameters,
	...
	public WmsRequest(HttpServletRequest request)
			throws MissingMandatoryParameterException {

		this(getList(request, "LAYERS", true), getList(request, "QUERY_LAYERS",
				false), getList(request, "STYLES", true), getBounds(
				getCrs(request), getParameter(request, "BBOX", true),
				getVersion(request)), getParameter(request, "FORMAT", true),
				Integer.parseInt(getParameter(request, "WIDTH", true)), Integer
						.parseInt(getParameter(request, "HEIGHT", true)),
				isTransparent(request), getBackgroundColor(request),
				getVersion(request),
				getParameter(request, "INFO_FORMAT", false),
				getParameters(request), getUrl(request), getTime(request));
	}
	...
	public WmsRequest modifySize(int w, int h) {
		return new WmsRequest(layers, queryLayers, styles, bounds, format, w,
				h, transparent, backgroundColor, version, infoFormat,
				parameters, url, time);
	}
	...
	public WmsRequest modifyStyles(List<String> newStyles) {
		return new WmsRequest(layers, queryLayers, newStyles, bounds, format,
				width, height, transparent, backgroundColor, version,
				infoFormat, parameters, url, time);
	}

	public WmsRequest modifyBounds(ProjectorBounds newBounds) {
		return new WmsRequest(layers, queryLayers, styles, newBounds, format,
				width, height, transparent, backgroundColor, version,
				infoFormat, parameters, url, time);
	}
	...
	private static boolean isTransparent(HttpServletRequest request)
			throws MissingMandatoryParameterException {
		return "true".equalsIgnoreCase(getParameter(request, "TRANSPARENT",
				false));
	}
	...
	private static String getCrs(HttpServletRequest request)
		...
		String crs = ObjectUtils.firstNonNull(
				RequestUtil.getParameter(request, "CRS", false), srs,
	...
	public String toString() {
		return "WmsRequest [layers=" + layers + ", queryLayers=" + queryLayers
				+ ", styles=" + styles + ", bounds=" + bounds + ", format="
				+ format + ", width=" + width + ", height=" + height
				+ ", transparent=" + transparent + ", backgroundColor="
				+ backgroundColor + ", version=" + version + ", infoFormat="
	 ...
	 * @return
	 */
	private static ProjectorBounds getBounds(String crs, String s,
			String version) {
		if (s == null)
			return null;
		...
		ProjectorBounds bounds = new ProjectorBounds(crs, minLong, minLat,
				maxLong, maxLat);
