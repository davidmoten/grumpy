 ...
 * <li><i>n</i> >= 5: the shape consists of segments, which are joined
 * together so that the first derivate is continuous at intermittent points
 * </ul>
 * 
 */
public class NearBSpline implements Shape, Cloneable {
	...
	public void setLine(double... points) {
		...
		for (int i = 2; i < points.length;) {
			...
			case 2:
				lastPart = new Line2D.Float((float) path.getCurrentPoint()
						.getX(), (float) path.getCurrentPoint().getY(),
				...
				break;
			case 4:
				lastPart = new QuadCurve2D.Float((float) path.getCurrentPoint()
						.getX(), (float) path.getCurrentPoint().getY(),
						(float) points[i], (float) points[i + 1],
						(float) points[i + 2], (float) points[i + 3]);
				path.append(lastPart, true);
				segments[i / 2 - 1] = lastPart;
				...
				break;
			case 6:
				lastPart = new CubicCurve2D.Double(path.getCurrentPoint()
						.getX(), path.getCurrentPoint().getY(), points[i],
			...
			default: // use two points and add one extra between 2nd and 3rd
				...
				lastPart = new CubicCurve2D.Double(path.getCurrentPoint()
						.getX(), path.getCurrentPoint().getY(), points[i],
						points[i + 1], points[i + 2], points[i + 3], x, y);
