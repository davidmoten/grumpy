...
public class Projector {
	...
	public com.vividsolutions.jts.geom.Point getGeometryPointInSrs(double lat,
			double lon) {
		Coordinate coordinate = new Coordinate(lon, lat);
		com.vividsolutions.jts.geom.Point point = geometryFactory
				.createPoint(coordinate);
		try {
			return (com.vividsolutions.jts.geom.Point) JTS.transform(point,
					transform);
	...
	public com.vividsolutions.jts.geom.Point getGeometryPointInSrsRelativeTo(
			double lat, double lon, double relativeLat, double relativeLon,
		...
		com.vividsolutions.jts.geom.Point point = getGeometryPointInSrs(lat,
				lon);
	...
	public double periodAtLat(double lat) {
		return getGeometryPointInSrs(lat, 180).getX()
	...
	public com.vividsolutions.jts.geom.Point getFirstXAfter(
			Projector projector, double lat, double lon, double x) {
		com.vividsolutions.jts.geom.Point point = projector
				.getGeometryPointInSrs(lat, lon);
