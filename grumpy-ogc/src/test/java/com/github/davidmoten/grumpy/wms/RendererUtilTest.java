...
public class RendererUtilTest {
	...
	public void testTransformWrapping() {
		...
		Point p2 = projector.getGeometryPointInSrsRelativeTo(-35, 141, -35,
				140, p.getX(), p.getY());
		System.out.println("x2=" + p2.getX());
		double x3 = p.getX() - projector.periodAtLat(-35);
		Double point3 = projector.getTargetPoint(projector.createPoint(x3,
				p.getY()));
		System.out.println("x3=" + x3 + " point=" + point3);
		double x4 = p.getX() + projector.periodAtLat(-35);
		Double point4 = projector.getTargetPoint(projector.createPoint(x4,
				p.getY()));
	...
	public void testGetCircle() {
		...
		ProjectorBounds bounds = new ProjectorBounds("EPSG:3857",
				14288114.828624, -6061227.593083, 18907357.32131,
				-2348222.5076192);
		Projector projector = new Projector(bounds, target);
